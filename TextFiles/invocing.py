...
def record_invoice(invoice_file: TextIO,
    ...
    for line in invoice_file:
        print('*', end='') # TODO delete this after testing
        last_row = line
    if last_row: 
    ...
    # if the file is empty, then we'll start numbering from 1
        year = get_year()
        new_invoice_number = f'{year}--{1:04d}'
        
    print(f'{new_invoice_number}\t{company}\t{amount}', file=invoice_file)
        
with open(r'C:\xampp\htdocs\Python\TextFiles\invoices.csv', 'r+') as data_file:
    record_invoice(data_file, 'AMCI', 20.34)
